...
def score_trajectory_2d(traj, actions, goal_xy,
                        ...
                        w_action=0.01, w_collision=10.0):   ##from 1.0 to 10.0
    """
    traj: (H+1, 4)
    actions: (H, 2)
    goal_xy: (2,)
    ...
    # ---- NEW: obstacle collision penalty ----
    d_obs = np.linalg.norm(xy_traj - OBSTACLE_CENTER, axis=1)

#    hit = d_obs < OBSTACLE_RADIUS
    # Large penalty if any state enters obstacle region
#    collision_penalty = 100.0 if np.any(hit) else 0.0

    # penalty if inside, stronger the deeper it is
    ...
    total_cost = (
        ...
        w_collision * collision_penalty ## now 10 * 100 = 1000
    )
...
def cem_plan_2d(
    ...
    goal_xy,
    H=20,          # was 10
    K=256,         # a bit more samples helps with obstacles
    elite_frac=0.2,
    iters=5,
