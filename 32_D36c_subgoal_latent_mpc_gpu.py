"""
32_D36c_subgoal_latent_mpc_gpu.py

D36c — Latent MPC on GPU + JEPA perception
...
This is intentionally a "toy latent planner":
    - It does NOT yet learn dynamics in JEPA space.
    - It DOES show GPU-parallel MPC over a latent state (x, y),
      which is the conceptual bridge to "latent JEPA MPC" later.
...
class LatentMPCPlanner:
    ...
    This is deliberately "small" but shows the core idea:
    GPU-parallel rollout over candidate futures in latent state space.
    """
    ...
    def _obstacle_tensors(self, obstacles: List[Obstacle]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        if len(obstacles) == 0:
            # handle degenerate case gracefully (no obstacles)
    ...
    def plan(
        ...
        # ---------------------------------------------
        # a_seq: (N, H, 2)
        # sample from N(0, 1), then scale to action_scale
        a_seq = torch.randn(self.num_candidates, self.horizon, 2, device=self.device)
        a_seq = a_seq * self.action_scale
        ...
        if centers.shape[0] > 0:
            # states: (N, H+1, 2)
            # centers: (O, 2)
            # we want pairwise distances (N, H+1, O)
            # unsqueeze to (N, H+1, 1, 2) and (1, 1, O, 2)
            ...
            # collision if "too close" at any point
            # we approximate by using global min_dist
            collision_mask = min_dist < (radii.max() + self.collision_margin) if radii.numel() > 0 else None
        ...
        # Debug print (optional, low frequency)
        print(
            f"[D36c] MPC best_idx={best_idx.item()}, "
            f"cost={total_cost[best_idx].item():.3f}, "
            f"dist_goal={dist_goal[best_idx].item():.3f}"
        )
...
def plot_trajectory(world: World, robot_path: List[Vec2], subgoal_seq: List[Vec2]) -> None:
    ...
    ax.set_title("D36c — Latent MPC (GPU) + JEPA perception")
    ax.legend()
    ax.grid(True)
    plt.show()
