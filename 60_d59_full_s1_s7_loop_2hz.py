# Filename: 60_d59_full_s1_s7_loop_2hz.py
# Purpose: D59 — Full Canonical RAI Loop (S1–S7 visible), 2 steps/sec
#
# This is a cleaned-up, complete script based on your D58 run.
# Key upgrades vs D58:
#   - 2 Hz default (dt=0.5)
#   - GIF saved at 2 fps
#   - Explicit S1–S7 logging every step (compact + consistent)
#   - Planner uses predicted belief (mu_pred, sigma_pred)
#
# Run:
#   python 60_d59_full_s1_s7_loop_2hz.py --steps 200 --out_gif d59.gif
#
# Optional:
#   python 60_d59_full_s1_s7_loop_2hz.py --steps 600 --fov_range 25 --obstacle 14,12,24,16 --out_gif d59_occlusion.gif
...
def plan_action(robot_xy, robot_theta, mu, sigma, cfg):
    """
    Returns (v, w, conf, dist, heading_err)
    Uses belief mean mu as aimpoint; uncertainty sigma modulates speed.
    """
    dx = float(mu[0] - robot_xy[0])
    dy = float(mu[1] - robot_xy[1])
    dist = math.sqrt(dx*dx + dy*dy)
    ...
    # confidence from sigma: small sigma => ~1, large sigma => ~0
    ...
    # Confidence floor so we still move to acquire when uncertain
...
def main():
    ...
    ap.add_argument("--conf_floor", type=float, default=0.35)       # keep moving when uncertain

    ap.add_argument("--obstacle", type=str, default="0,28,2,30")
    ap.add_argument("--no_occlusion", action="store_true")
    ap.add_argument("--out_gif", type=str, default="d59_full_loop.gif")
    ...
    for t in range(cfg.steps):
        ...
        # -------------------------
        # S4) Plan action from predicted belief
        # -------------------------
        v, w, conf, dist, heading_err = plan_action(robot_xy, robot_theta, mu_pred, sigma_pred, cfg)
        a_t = (v, w)
        ...
        # S1–S7 visible log (every step)
        print(
            f"[t={t:03d}] "
            f"S1 x={('None' if x_t is None else fmt2(x_t))} vis={int(vis)} | "
            f"S2 z={('None' if z_t is None else fmt2(z_t))} | "
            f"S3 Bpred=(mu={fmt2(mu_pred)}, s={sigma_pred:4.2f}) | "
            f"S4 a=(v={v:4.2f}, w={w:4.2f}) conf={conf:4.2f} | "
    ...
    ax.set_aspect("equal")
    ax.set_title("D59 — Full S1–S7 Loop (2 Hz)")
    ...
    def update(frame):
        ...
        # approximate heading from finite diff (simple + robust)
    ...
    print(f"[D59] Saving GIF -> {cfg.out_gif}")
