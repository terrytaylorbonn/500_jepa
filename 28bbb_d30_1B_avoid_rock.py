# 28bbb_d30_1B_avoid_rock.py
import numpy as np
import matplotlib.pyplot as plt
import imageio.v2 as imageio

# ==========================
# Obstacle (world coords)
# ==========================

OBSTACLE_CENTER = np.array([-1.0, -1.0], dtype=np.float32)
OBSTACLE_RADIUS = 1.0  # world units
# Start = (-4, -4), goal = (3, 2) → obstacle at (-1, -1) is nicely in the way.
...
# ===================================
# 2. Cost: terminal, path, effort, obstacle
# ===================================
...
def score_trajectory_2d(traj, actions, goal_xy,
    ...
    # ---- Smooth obstacle penalty ----
    d_obs = np.linalg.norm(xy_traj - OBSTACLE_CENTER, axis=1)
    # penetration depth inside obstacle (0 if outside)
    penetration = np.clip(OBSTACLE_RADIUS - d_obs, 0.0, None)
    # square penalty for deeper penetration, scaled
    collision_penalty = np.sum(penetration ** 2) * 50.0
...
def cem_plan_2d(
    ...
    elite_frac=0.2,
    iters=5,
    init_std=0.7,  # slightly smaller initial exploration
    dt=0.1,
):
    """
    D30-minimal planner:
...
def render_world_to_grid(state, goal_xy, grid_size=32):
    ...
    # Obstacle in world coords → pixel
    ox, oy = to_pix(OBSTACLE_CENTER)

    img[ry, rx] = 1  # robot
    img[gy, gx] = 2  # goal
    img[oy, ox] = 3  # obstacle (center pixel)
...
# ===================================
# 5. Main loop (D30 minimal + GIF/plot)
# ===================================
...
if __name__ == "__main__":
    ...
    T = 40  # more steps to see the full path
    ...
    for t in range(T):
        ...
        d_obs_now = np.linalg.norm(state[:2] - OBSTACLE_CENTER)
        ...
        print(
            ...
            f"d_goal={d_goal:0.3f}  d_obs={d_obs_now:0.3f}"
        )
    ...
    # obstacle
    circle = plt.Circle(OBSTACLE_CENTER, OBSTACLE_RADIUS, fill=False)
    ax.add_patch(circle)
    ...
    ax.set_title("D30 1B: path with obstacle")
    ax.legend()
    plt.tight_layout()
    plt.savefig("d30_1B_path.png", dpi=150)
    plt.close(fig)
    ...
    for i in range(1, len(traj_xy) + 1):
        ...
        ax.set_yticks([])
        plt.tight_layout()
        frame_path = "frame_tmp.png"
        plt.savefig(frame_path, dpi=100)
        plt.close(fig)

        frames.append(imageio.imread(frame_path))

    imageio.mimsave("d30_1B_path.gif", frames, fps=5)
    print("Saved d30_1B_path.png and d30_1B_path.gif")
