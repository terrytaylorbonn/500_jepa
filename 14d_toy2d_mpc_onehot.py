"""
14d_toy2d_mpc_onehot.py

Closed-loop MPC in a simple Toy2D environment using:

- Frozen I-JEPA encoder (facebook/ijepa_vith14_1k)
- One-hot latent predictor trained in 14b

Algorithm:
  1) Sample a random goal grid position and render goal image x_goal.
  2) Loop t = 0..T:
       - Encode current obs x_t -> z_t
       - Random-shooting planner from (z_t, z_goal) proposes best action seq
       - Execute FIRST action only in the real env
       - Stop if latent distance ||z_t - z_goal|| is below threshold
"""
...
def mpc_rollout(
    ...
    """
    Run closed-loop MPC and return list of frames (PIL Images).
    """
    ...
    while step < max_env_steps:
        ...
        # Check latent distance to goal for early stop
        with torch.no_grad():
            z_cur = encode_image_to_latent(cur_img, encoder, processor, device)
            dist = torch.norm(z_cur - z_goal, dim=-1).item()
        print(f"[14d] step={step} latent_dist={dist:.4f}", flush=True)

        if dist < stop_threshold:
            print(f"[14d] Stopping: latent distance {dist:.4f} < {stop_threshold}", flush=True)
            break

        if done:
            print(f"[14d] Env done at step {step}", flush=True)
            break
