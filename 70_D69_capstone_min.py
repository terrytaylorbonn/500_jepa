...
def parse_args():
    ...
    # NEW: cooldown to prevent rapid LOST/REACQUIRED chatter ###################
    p.add_argument("--vis_cooldown", type=int, default=10)
    ...
    ##3b vis debounce + occlusion damping
...
def sweep_theta(theta0, t, occ_run, amp_deg=150.0, period=120):
    amp = deg2rad(amp_deg)
...
def main():
    ...
    # true target init (capstone: start visible)
    ...
    ##10b visibility debounce state (prevents LOST/REACQUIRED flicker)
    vis_state = False
    vis_on_run = 0
    vis_off_run = 0
    # NEW: lockout timer to stop flip-flop ############################
    vis_lock_until = -1
    ...
    print(f"  vis_on_n/off_n={args.vis_on_n}/{args.vis_off_n} occ_vel_damp={args.occ_vel_damp}")
    ...
    for t in range(args.steps):
        ...
        theta_use = theta0 if occ_run == 0 else sweep_theta(theta0, t, occ_run, args.sweep_amp_deg, args.sweep_period)
        raw_vis = visible(robot_xy, theta_use, x, args.fov_range, fov_half_angle, obstacle)

        ##50c1 debounce raw visibility into stable vis_state
        if raw_vis:
            vis_on_run += 1
            vis_off_run = 0
        else:
        ...
        elif can_flip and vis_state and (vis_off_run >= args.vis_off_n):
            vis_state = False
            vis_lock_until = t + args.vis_cooldown
        ...
        if not vis:
            ...
            ##51a2 predict + extra velocity damping (prevents runaway)
            mu_x = mu_xp
            ...
            if occ_run == args.search_after:
                ##51a1 search trigger (+ reset velocity)
            ...
            if prev_mode is None:
                prev_mode = mode
            if mode != prev_mode:
                ##51b1 narrative mode change
                print(f"[t={t:04d}] EVENT: mode -> {mode_names[mode]}")
                prev_mode = mode
